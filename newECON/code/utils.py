...
def convert_examples_to_features_te(story_dir, tokenizer, max_seq_length,
                                            is_training, includeIds=None, analyze=False,data=None,major_event=False):
    """
    story_dir: a directory that contains a data.pickle file, which has the formal defined Matres data format.
    if major event, then search for major_data.pickle
    ...
    if data:
        ...
        if major_event:
            examples = pickle.load(open(path.join(story_dir, "major_data.pickle") , "rb"))
        else:
