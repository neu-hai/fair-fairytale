        ...
        with torch.no_grad():
            ...
            with open(filename) as file:
                ...
                if self.doCoref:
                    ...
                    non_characters = ["humans","human","human beings","human being","no one","some one", "someone", "one","nobody","no body","anybody","any body", "anyone","any one","some","each one","the others","all","anyone else",
                                     "nobody else","no one else","the latter"]
                    characters = {}
                    pronouns = {}
                    for a, e in zip(assignments, entities):
                        cat = e[2]
                        ...
                        ner_type=cat.split("_")[1]
                        if ner_type == "PER":
                            if (ner_prop =="NOM" or ner_prop =="PROP") and (e[3].lower() not in non_characters):
                                #each character contains [ls, int]
                                #a list of cluster names, and also integer count of all name clusters                                                 
                                if (ner_prop =="NOM" or ner_prop =="PROP"):
                                        if a not in characters:
                                            characters[a] = [set([e[3]]),1]
                                        else:
                                            characters[a][0].add(e[3])
                                            characters[a][1]+=1
                                        pronouns[a] = Counter()

                    #this dictionary pronouns is keeping track of the number of pronouns mentions associated with a certain character
                    for a, e in zip(assignments, entities):
                        cat = e[2]
                        ner_prop=cat.split("_")[0]
                        ner_type=cat.split("_")[1]
                        if ner_type == "PER":
                            if (ner_prop =="PRON") and (e[3].lower() not in non_characters):
                                if a in pronouns:
                                    pronouns[a][e[3]]+=1

                    #here, we can try to write a big file containing the meta information about referenced characters, including
                    #clustered names, reference number, names mention count, pronoun mention, total mention count
                    with open(join(outFolder, "%s.character_meta" % (idd)), "w", encoding="utf-8") as out:
                        out.write("clustered_names\tcoref_idx\tname_mentions\tpronoun_mentions\ttotal\n")
                        for ref in list(characters):
                            cha_text, cha_cnt = "/".join(characters[ref][0]),characters[ref][1]
                            pron_cnt = sum(pronouns[ref].values())
                            total = cha_cnt+pron_cnt 
                            out.write("%s\t%s\t%s\t%s\t%s\n" % (cha_text, ref, cha_cnt, pron_cnt, total))
...
#                   print(ent_names)
                ...
                #arrange the ranked, and pron-labeled character meta output
                if self.doEntities and self.doCoref:

                    meta_path = join(outFolder, "%s.character_meta" % (idd))
                    ent_path = join(outFolder, "%s.entities" % (idd))
                
                    
                    df = pd.read_csv(ent_path, sep="\t")
                    df1 = pd.read_csv(meta_path, sep="\t")
                    ...
                    for i in range(df.shape[0]):
                        ...
                        if i>0 and int(df.iloc[i-1]["start_token"]) >= int(start) and int(df.iloc[i-1]["end_token"])<=int(end) :
                            ...
                            if cat == "PER" and (prop =="NOM" or prop =="PROP"):
                                if df.iloc[i-1]["prop"]=="PRON" and df.iloc[i-1]["cat"]=="PER":
                                    ...
                                    #if the pronoun can be coreferred to a NOM term, then we will update the name
                                    ...
                                    if len(pron_ls)==1:
                                        ...
                                        # we should definitely be able to find a colomn in df1, with this coref idx:
                                        idx = df1.index[df1['coref_idx'] == coref].tolist()[0]
                                        if (idx not in name_dict) or (
                                                idx in name_dict and df1.iloc[idx]["clustered_names"] not in name_dict[idx]):
                        ...
                        elif i+1<df.shape[0] and int(df.iloc[i+1]["start_token"]) >= int(start) and int(df.iloc[i+1]["end_token"])<=int(end) :
                            #if the current token is a PER NOM, and prev a PER,PRON
                            if cat == "PER" and (prop =="NOM" or prop =="PROP"):
                                if df.iloc[i+1]["prop"]=="PRON" and df.iloc[i+1]["cat"]=="PER":
                                    #if all thos conditions are satisfied, we need to make change to the name of this coref
                                    ...
                                    if len(pron_ls)==1:
                                        ...
                                        if idx not in name_dict:
                                            ...
                                            name_dict[idx].add(df1.iloc[idx]["clustered_names"])
                    for i in range(len(easy_name)):
                        easy_name[i] = easy_name[i].split("/")[0]
                    df1["easy_name"] = easy_name
                    df1 = df1.sort_values("total", ascending=False)
                    df1.to_csv(meta_path+"_pron",index=False,sep="\t")
